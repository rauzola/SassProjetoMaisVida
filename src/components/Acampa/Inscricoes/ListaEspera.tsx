...
const CACHE_KEY = "acampa-lista-espera-cache";
...
export function AcampaListaEspera() {
  ...
  const fetchCampistas = async (force = false) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('Acampa1CampistaCorpusChristi2025FichaDeEspera')
        .select('*')
      ...
      <CardHeader className="bg-gradient-to-r from-indigo-50 to-purple-50 border-b px-6 py-5">
        <div className="flex flex-col gap-1">
          <CardTitle className="text-2xl font-bold text-slate-800">
            Lista de Espera Acampa Corpus Christi 2025
          </CardTitle>
